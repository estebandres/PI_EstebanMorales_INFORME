...
Para la configuración inicial el módulo genera un Access Point WiFi, el teléfono android se conectará a éste generando una LAN IPv4 con solo estos dos dispositivos. Utilizando esta red la aplicación enviará las credenciales del router WiFi para operación regular.

El modo de operación regular del producto requiere que el módulo electrónico y el teléfono android con la aplicación estén conectados al mismo router WiFi. De esta manera ambos dispositivos pertenecen a la misma LAN IPv4.
El módulo publicará su hostname, nombre de servicio y metadata adicional empleando los protocolos de 
Como parte del diseño del sistema se eligió el protocolo HTTP para el intercambio de mensajes entre el módulo y la aplicación móvil. Ya sea para la configuración inicial o la operación regular.
...
En una segunda etapa se extenderá el modo de comunicación entre el módulo y la aplicación móvil a través de internet empleando el protocolo MQTT.
...
\end{itemize}
En mDNS/DNS-SD, todos los RRs que describen una instancia de servicio se almacenan en el nodo que proporciona tal servicio.

\textbf{Bonjour} es una implementación zeroconf desarrollada por Apple. Emplea mDNS como protocolo de comunicación y DNS-SD para el descubrimiento y descripción de servicios.
...
Android ofrece una API para el descubrimiento de dispositvos \emph{Network Service Discovery API} pero ésta no soporta la consulta de registros TXT. Google ofrece también la \emph{Google Nearby API} una librería de similares características pero que es de código cerrado y no proporciona el nivel de granularidad adecuado para este proyecto. Por estos motivos se optó por emplear la librería \emph{RxDNSSD} que hace de wrapper reactivo sobre la implementación de Bonjour.

\subsubsection{HTTP y Digest Authentication}
\subsubsection{MQTT}
Uno de los atractivos del producto es la posibilidad de monitorear y accionar módulos de manera remota a través de internet. Para que esto sea posible es necesario que tanto el módulo como la aplicación móvil tengan acceso a la nube que hará de canal de comunicación entre ambos.
Luego de discutir alternativas el equipo decidió que la comunicación por internet se realizará utilizando el protocolo MQTT.
MQTT es un protocolo de capa de aplicación (RFC 9431) 
